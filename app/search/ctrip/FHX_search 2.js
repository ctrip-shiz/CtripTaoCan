(function(){
    ...
    var Mod = {
        ...
        showTips: function (obj, msg, hideEvent) {
            if(!Mod.formValidator)
                Mod.formValidator = $(document).regMod("validate", PluginsVersion['validate']);
	...
	var HistoryKeeper = {
		...
		recover: function (){
			...
			$('#FHX_divHotelQuery input').each(function (input, i){
				switch(i){
	...
	var Searcher = {
		...
		_updateHotelBox: function (dates) {
			...
			deletelnks[0].className = deletelnks.length > 1 ? 's_item_del fr' : 's_item_del fr hidden';
			deletelnks.length >=3 ? $('#FHX_lnkAddHotel').addClass('hidden') : $('#FHX_lnkAddHotel').removeClass('hidden');
			
			//This is bullshit.
			var heights = [286, 326, 340], contentHeights = [261, 301, 315];
			$('#searchBox').css({height: heights[this._hotelCount-1] + 'px'});
			$('#searchBoxUl').css({height: heights[this._hotelCount-1] + 'px'});
			$('#FHX_container').css({height: contentHeights[this._hotelCount-1] + 'px'});
		},
		...
		_initMod: function (){
			...
			var selectAdult = this.selectAdult = $('#FHX_AdultSelect');
			var selectChildren = this.selectChildren = $('#FHX_ChildrenSelect');
		...
		_initFlightMods: function () {
			...
			var tempAddress = 
				self.txtFrom.regMod('address', PluginsVersion['address'], {
					name: 'txtFrom',
					isAutoCorrect: true,
			...
			tempAddress =
				self.txtTo.regMod('address', PluginsVersion['address'], {
					name: 'txtTo',
					isAutoCorrect: true,
			...
			var tempCal = 
				self.txtFromDate.regMod('calendar', PluginsVersion['calendar'], {
					options: {
			...
			tempCal = 
				self.txtToDate.regMod('calendar', PluginsVersion['calendar'], {
					options: {
		...
		_initHotelMods: function (isReset) {
			...
			$('#FHX_lnkAddHotel').bind('click', self.addHotel.bind(self));
		},
		...
		_regHotelMods: function (txt, i) {
			...
			if(which == 0){ // reg address
				...
				if(which == 1){
					var tempCal =
						txt.regMod('calendar', PluginsVersion['calendar'], {
							options: {
								minDate: self.txtFromDate.value().toDate().addDays(-1).toStdDateString(),
	...
	FHX = {
		init: function (){
			// create style
			var styleStr = ".hidden{display:none;} ";
	...
	// 设置出发城市焦点（自由行tab被点击时调用）
	FHX.freeSF = function (){
	
		if(!Searcher.txtFrom){
			var txtFrom = $('#FHX_txtFrom')[0];
			txtFrom.focus();
			txtFrom.select();
			return;
		}
	
		Searcher.txtFrom.address.method('focus',{
			isHidden:true,
			isSelected:true
		});
	}
