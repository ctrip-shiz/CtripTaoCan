...
(function (exports) {
    ...
    // 全局表单
    var fm = document.getElementById('airHotelSearchForm');//for home
    ...
    var Mod = {
        ...
        formValidator: $(fm).regMod("validate", "1.1"),
        // show invalid tips
    ...
    var FH = {
        ...
        init: function () {
            ...
            //搜索
            // $('#FH_indexBtnSearch').bind('click', function (e) {
                // // 设置是否是特价信息的搜索
                // FH.search();
				// // for home
				// e.preventDefault();
        ...
        search: function (searchType) {
            ...
            if (FH._searchType == SEARCH_TYPE.FREE_COMBINE) {
				...
				if (FH._searchType == SEARCH_TYPE.ROUND)
					$('#FH_hNight').value(
						S.dateDiff(OneRounder.txtDDate.value(), OneRounder.txtRDate.value(), 'd'));
    ...
    var OneRounder = {
        ...
        generateUrl: function () {
            ...
            if (this.txtRDate.value() == '')
                searchType = 'oneway';
			// for home
			FHConfig.URL.FLTHOTEL_SEARCH = window.location.protocol + '//' + window.location.host.replace(/\w+\./, 'package.') + '/$1-$2$3-$4$5/';
        ...
        switchDateDisplay: function (isOneWay, isInit) {
            ...
            if (isOneWay) {
				...
				$('#FH_RoomSelect').addClass('w02');
				Mod.setCalValue(this.txtRDate, '');
            } else {
                S.show(this.txtRDate.parentNode());
                S.hide(this.selectNight);
				$('#FH_RoomSelect').removeClass('w02');
		...
		save: function (key, opts){
			...
			// cQuery.cookie.set(key, 'checkIn', this.txtCheckInDate.value(), opts); // for home
			...
			cQuery.cookie.set(key, 'night', $('#FH_hNight').value(), opts);
		},
		//恢复最近一次单程往返搜索
		recover: function (key){
			...
			$('#FH_hNight')[0].value = cQuery.cookie.get(key, 'night') || $('#FH_hNight')[0].value;
        ...
        _initDateMod: function (txtDDate, txtRDate, txtCheckInDate, txtCheckOutDate) {
            ...
            calendars[0] = txtDDate.regMod('calendar', '3.0', {
                options: {
                    startDate: new Date().addDays(1).toStdDateString(),
